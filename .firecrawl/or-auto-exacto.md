...
Without Auto Exacto, OpenRouter’s default routing is primarily [price-weighted](https://openrouter.ai/docs/guides/routing/provider-selection#price-based-load-balancing-default-strategy) — requests are load balanced across providers with a strong preference for lower cost. Auto Exacto changes this for tool-calling requests by reordering providers based on quality signals instead of price.
...
- **`provider.sort` parameter** — set `sort` to `"price"` in the `provider` object of your request body. See [Provider Sorting](https://openrouter.ai/docs/guides/routing/provider-selection#provider-sorting) for details.
- **`:floor` virtual variant** — append `:floor` to any model slug (e.g. `openai/gpt-4o:floor`) to sort by price. See [Floor Price Shortcut](https://openrouter.ai/docs/guides/routing/provider-selection#floor-price-shortcut).
